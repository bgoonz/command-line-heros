...
rh.webux.dynamicForm = {
    ...
    isLoaded:   function() {
        return ( $( "script[src*='jquery.gatedform.min.js']" ).length > 0 ) ||
    ...
    validateForm: function( $el ) {
        var myself = this;
        if ( $el.val() !== "" ) {
            $el.parents( myself.group ).removeClass( "df-has-error" ).addClass( "df-correct" );
        } else {
            $el.parents( myself.group ).addClass( "df-has-error" ).removeClass( "df-correct" );
        }
    },
    ...
    setHiddenFields: function( queryObj ) {
        ...
        for ( var key in queryObj ) {
            ...
            // Add the question
            myself.createHiddenField( key, count, key, "Question" ).appendTo( $parent );
            // Add the answer
            myself.createHiddenField( key, count, queryObj[ key ], "Answer" ).appendTo( $parent );
        }
    }
};
...
if ( document.location.hostname === "localhost" ) {
    rh.webux.dynamicForm.addScript( "//www.redhat.com/forms/scripts/jquery.gatedform.js" );
} else {
...
$( "a[href^=#]" + rh.webux.smoothScroll.notStrings, context ).click( function( event ) {
...
rh.webux.slider = {
    ...
    isVaryHeight: function( $parent ) {
        return $parent.has( this.switcher.slide )
            .attr( this.attr.height ) == "variable";
    },
    ...
    setParentHeight: function( context ) {
        var myself = this,
            ...
            id, // String
            max, height, // Number
            isVariable, isCurrent; // Boolean
        $( myself.nav.parent, context )
            .each( function( i, val ) {
                id = myself.getID( $( val ) );
                $parent = $( "[" + myself.attr.id + "='" + id + "']", context );
                max = 0;
                $parent.children( myself.switcher.slide )
                    .each( function( j, slides ) {
                        $slide = $( slides );
                        $body = $parent.has( myself.switcher.slide );
                        height = $slide.height();
                        isVariable = myself.isVaryHeight( $slide.parent( myself.switcher.body ) );
    ...
    go: function( id, to, direction, context ) {
        ...
        $.each( myself.nav.styles, function( i, style ) {
            styles[ style ] = $parts.parent( "[" + myself.attr.style + "='" + style + "']" );
        } );

        // Update navigation styles
        $.each( [ styles.circles, styles.text ], function( i, nav ) {
            $( nav ).find( myself.nav.parent ).children().each( function( j, item ) {
                this.active = $( item ).attr( myself.attr.to ) == to ? "true" : "false";
        ...
        styles.arrows.each( function( i, nav ) {
            ...
            if ( this.loc > -1 ) {
                ...
                this.newID = ( this.active ) ?
                    contentIDs[ this.iterate ] : contentIDs[ this.limit ];
                /* Update arrow data and hide or show arrows
                    depending on content available next or prev */
                this.$arrow.attr( myself.attr.to, this.newID )
        ...
        if ( typeof direction != "undefined" ) {
            $last = $moveFrom;
            var stack = ( direction == "prev" ) ? $moveFrom.prevAll() : $moveFrom.nextAll();
            var i = 0;
...
window.DynamicFormConfig = {
    ...
    "language": "en",
    "FormIntro": "The story is still unfolding. Sign up and we'll notify you when it premieres.",
    "NameOrder": "western",
...
$( "document" ).ready( function() {
    ...
    function openChapter( element, event ) {
        ...
        // Attributes
        var $target_parent = $( "[data-ux-trigger-parent='" + target_attr + "']" );
        // States
    ...
    $( window ).on( "scroll", function() {
        ...
        $( ".page-background-image" ).css( "background-color", "rgb(" + rgb3.join() + ")" );
    } );
